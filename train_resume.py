...
CKPT_DIR   = "ckpt_best"
PRED_PATH  = "predictions_neural.json"
...
def main():
    ...
    class_weights = compute_class_weights(train_c)
    ...
    ckpt_to_resume = "ckpt_best/checkpoint-79" if os.path.isdir("ckpt_best/checkpoint-79") else None
    print(f"Resuming from checkpoint: {ckpt_to_resume}")

    model = AutoModelForTokenClassification.from_pretrained(
        ckpt_to_resume if ckpt_to_resume else MODEL,
    ...
    print(f"Starting training (resuming from {ckpt_to_resume})...")
    trainer.train(resume_from_checkpoint=ckpt_to_resume)

    print("\n--- Evaluating Best Model on Local Validation ---")
    ...
    json.dump(test_results, open("predictions.json", "w"), indent=2)
    print(f"Saved {len(test_results)} predictions -> {PRED_PATH} and predictions.json")
